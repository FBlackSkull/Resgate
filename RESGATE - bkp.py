...
###########################################################################################################
######################### ATUALIZANDO PYTHON E INSTALANDO PARAMETROS PRIMARIOS ############################
###########################################################################################################
import os, time, sys
...
# OS
g_OS = input(colored("NUMERO DA OS? ", 'cyan', attrs=['bold']))
# NUMERO DE SERIE
g_NS = input(colored("NUMERO DE SERIE? ", 'cyan', attrs=['bold']))
...
############################################# CONECTA PELA SENHA ##########################################
###########################################################################################################
def ssh_connect_password(pwdNum):
    ...
    try:
        #client.connect(lastValidHost, port=validPort, username=g_users, password=g_passwords[str(pwdNum)], timeout=1.0)
        client.connect(lastValidHost, username=g_users, password=g_users[str(pwdNum)], timeout=2.0)
...
def ssh_connect_key(disabled_alg=None) -> bool:
    ...
    client.load_system_host_keys()
    client.set_missing_host_key_policy(AutoAddPolicy())
    g_SSH = paramiko.RSAKey.from_private_key_file(g_C_D+'\\BKP\\PRIVATEKEY', password=g_passphrase)

    try:
        client.connect(lastValidHost, username=g_users, pkey=g_SSH, port=validPort, timeout=1, disabled_algorithms=disabled_alg)
        ssh_sessions.append(client)
        return True
    except Exception as e:
        print(str(e))
...
def thread_get_ip(ip, port):
    ...
    try:
        s = socket.create_connection(address=(ip, port), timeout=1.0)
        s.close()
        lastValidHost = ip
        validPort = port
    except:
...
def windows_is_local_networked_host(ip) -> bool:
    """
    params:
        ip: remote host passed in as string. Ex: "192.168.1.1"
    returns: boolean
    """

    try:
        output = system_call(f"ping -4 -n 1 -i 1 -w 50 {ip}") # TTL is set to 1. So if remote host is within another network, the ICMP/ping packet is dropped.
    except Exception as e:
        print(str(e))
        return False
...
def get_ssh_session() -> SSHClient:
    # Tentar estabelecer conexao com o numero maximo de senhas registradas no arquivos config.json
    global ssh_sessions, g_passwords
    ssh_sessions = []

    if(ssh_connect_key({'pubkeys': ['rsa-sha2-256', 'rsa-sha2-512']})):
        #print("SSH sem public keys.")
        pass
    elif(ssh_connect_key()):
        #print("SSH com public keys.")
        pass
    else:
        #print("SSH mediante a usuário e senha.")
        threads = []
        index = 0
        for p in range(len(g_passwords)):
            t = threading.Thread(target=ssh_connect_password, args=(p))
            threads.append(t)
            t.start()
            index = index + 1
...
def conexao_falhou():
    ...
    if platform.system() == "Windows":
        if os.path.exists(g_C_D+f'\\BKP\\RESGATE-{ANO}.xlsx'):
            ...
            TITULO = ['OK', 'EMPRESA', 'O. DE SERV.', 'SERIAL', 'TEM ARQUIVOS', 'SEQUENCIAL DOS ARQUIVOS', 'DATA', 'OBSERVAÃÃO', 'DEFEITO CONSTATADO']
        ...
        LOCALIZADOR.write('\n-> EQUIPAMENTO COM SOFTWARE BLOQUEADO')
        LOCALIZADOR.close()
        ### CARREGANDO PLANILHA DE RESGATE ###
        RESGATE = load_workbook(g_C_D+f'\\BKP\\RESGATE-{ANO}.xlsx')
        PLANILHA = RESGATE['INTERVENCAO']
        ### ADICONANDO INFORMAÇÕES NO ARQUIVO XLSX ###
        RESGATE_INFO = ['F', g_EMPRESA, g_OS, g_NS, 'NAO', ' ', DATAXLSX, 'EQUIPAMENTO COM SOFTWARE BLOQUEADO']
        PLANILHA.append(RESGATE_INFO)
        RESGATE.save(g_C_D+f'\\BKP\\RESGATE-{ANO}.xlsx')
        print(colored('\n   ADICIONADA INFORMAÇÃO NA PLANILHA!', 'green'))
...
def windows_processar(client: SSHClient):
    ...
    if BK >= 0:
            ...
            if SD == -1:
                ...
                if EXECUTAVELY[:-1] == "SPTrans-v3068":
                    ...
                    if VERFSD[0:35] == "/media/mmcblk0p1/opt/v750/system/ud":
                        ...
                        if KB == -1:
                            ...
                            if (int(KB_00000[:-19])) <= 8000:
                                ...
                                print(colored('\n    --> INICIANDO RESGATE DO ARQUIVO BKP_File_002_00000 <--', 'grenn', attrs=['bold']))
                                stdin, stdout, stderr = client.exec_command("cd /media/mmcblk0p1/opt/v750/system/ud\n"f"/root/app/{EXECUTAVEL_A} -R")
                                time.sleep(2)
                                stdin, stdout, stderr = client.exec_command("cd /media/mmcblk0p1/opt/v750/system/ud\n""ls TG*")
                    ...
                    print(colored('---> VALIDADOR FALTANDO APLICAÇÃO OU APP NAO IDENTICIADO!!! <---', 'red'))
            ...
            RESGATE_INFO = ['F', g_EMPRESA, g_OS, g_NS, INFO_PCGAR_TG+INFO_PCGAR_BOM, SEQUENCIAL_INFO + SEQUENCIAL_PCGAR + SEQUENCIAL_INFO2, DATAXLSX, SD_PLAN + BKP_PLAN, OBS_PLAN]
            PLANILHA.append(RESGATE_INFO)
            RESGATE.save(g_C_D+f'\\BKP\\RESGATE-{ANO}.xlsx')
        ...
        ######################## EXIBIR ARQUIVOS EXTRAIDOS ########################
            ...
            ### VERIFICAR SE REINICIA OU NAO ###
            print(colored("\n\nREINICIAR?", 'yellow'))
            REINICARBKP = input('Digite S para SIM ou ENTER para Fechar...\n').upper()
            if REINICARBKP == 'S':
                print('REINICIANDO...')
                os.system('python RESGATE.py')
            elif REINICARBKP == 'SIM':
                print('REINICIANDO...')
                os.system('python RESGATE.py')
            else:
                print('FECHANDO...\n')
    ...
    if EXECUTAVEL1[:-1] == "SPTrans-v3068":
            ...
            print(colored('Nada a resgatar... Finalizando aplicação...', attrs=['bold']))
            ...
            PLANILHA.append(RESGATE_INFO)
            RESGATE.save(g_C_D+f'\\BKP\\RESGATE-{ANO}.xlsx')
            print(colored("\n   --> FECHANDO CONEXAO SSH. <---", 'red', attrs=['bold']))
            client.close()

            ### VERIFICAR SE REINICIA OU NAO ###
            print(colored("\n\nREINICIAR?", 'yellow'))
            REINICARFABRICA = input('Digite S para SIM ou ENTER para Fechar...\n').upper()
            if REINICARFABRICA == 'S':
                print('REINICIANDO...')
                sys.stdout.flush()
                os.system('python RESGATE.py')
            elif REINICARFABRICA == 'SIM':
                print('REINICIANDO...')
                sys.stdout.flush()
                os.system('python RESGATE.py')
            else:
    ...
    if g_SP_CMT2 == 'SPTRANS':
        ...
        if SERIAL_SERIALREADER >= 0:
                ...
                if SERIAL_READER >= 0:
                    ...
                    print(colored('SERIAL NAO ENCONTRADO!', 'red', attrs=['bold']))
                    print('')
    ...
    ### VERIFICANDO ARQUIVOS TG ###
            ...
            if (TG[:2]) == 'TG':
                print(colored("\nSALVANDO OS ARQUIVOS TG EXISTENTES!!!", 'green'))
                ssh_command(client, "cd /opt/v750/system/ud\n""tar -cf TG.tar.gz TG*")
                print('DOWNLOAD: ...', end='')
                time.sleep(2)
                ### DOWNLOAD DO ARQUIVO ###
    ...
    ############################################## TOP - BOM #####################################################
            ...
            if (TG[:2]) == 'TG':
                ...
                print(colored('\nSEM ARQUIVOS TG-TOP PARA RESGATAR!!!\n', 'red'))
                INFO_PCGAR_TG = 'NAO'
                SEQUENCIAL_INFO = ' '
                time.sleep(2)
            ...
            if (TG_BOM[:2]) == 'TG':
                ...
                print(colored('\nSEM ARQUIVOS TG-BOM PARA RESGATAR!!!\n', 'red'))
                SEQUENCIAL_BOM = ' '
                time.sleep(2)
    ...
    ### ADCIONINANDO INFORMACOES NA PLANILHA ###
    RESGATE_INFO = ['F', g_EMPRESA, g_OS, g_NS, INFO_PCGAR_TG + INFO_PCGAR_BOM,SEQUENCIAL_INFO + SEQUENCIAL_PCGAR + SEQUENCIAL_BOM, DATAXLSX, SERIAL_PLAN + SD_PLAN + BKP_PLAN, OBS_PLAN]
    PLANILHA.append(RESGATE_INFO)
    RESGATE.save(g_C_D+f'\\BKP\\RESGATE-{ANO}.xlsx')
    ...
    print(INFO_PCGAR_TG + INFO_TG + INFO_BOM)
...
def main():
    ...
    if(validPort == -1):
        #print(colored(f'FALHA NA COMUNICAÇÃO. VERIFIQUE O VALIDADOR OU CONEXAO DE REDE.', 'yellow'))
        conexao_falhou()
        return
    ...
    if(not is_same_network):
        #print(colored("FALHA NA COMUNICAÇÃO. O VALIDADOR ", 'yellow'), end="")
        #print(f"{lastValidHost}", end="")
        #print(colored("NÃO ESTA NA MESMA REDE QUE ESTE COMPUTADOR", 'yellow'))
        conexao_falhou()
        return
